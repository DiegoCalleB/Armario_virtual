...
import { Calendar, Sun, Cloud, CloudRain, CloudLightning, Thermometer, Sparkles, Plus, Trash2, Check, ArrowRight, Eye, RefreshCw, Shirt, HelpCircle, Briefcase } from "lucide-react";
...
const CLIMAS_INFO = {
  soleado: { icon: Sun, color: "text-amber-500", bg: "bg-amber-500/10", label: "Soleado", desc: "Día despejado, ideal para prendas transpirables y accesorios como gafas de sol." },
  nublado: { icon: Cloud, color: "text-blue-400", bg: "bg-blue-500/10", label: "Nublado", desc: "Cielo cubierto. Ideal para looks de entretiempo con capas ligeras." },
  lluvioso: { icon: CloudRain, color: "text-sky-500", bg: "bg-sky-500/10", label: "Lluvioso", desc: "Precipitación activa. Se requiere calzado impermeable, gabardina o cortavientos." },
  tormenta: { icon: CloudLightning, color: "text-purple-500", bg: "bg-purple-500/10", label: "Tormenta", desc: "Clima adverso. Elige abrigos protectores, calzado cerrado de cuero y evita prendas delicadas de ante o seda." },
  frio: { icon: Thermometer, color: "text-teal-500", bg: "bg-teal-500/10", label: "Frío Extremo", desc: "Temperaturas bajo mínimos. Prioriza lana merina, abrigos estructurados gruesos y bufandas." }
};
...
export default function PlanificadorLooks({
  ...
  // Office days recurrent configuration (Persisted in localStorage)
  const [configOficina, setConfigOficina] = useState(() => {
    const cached = localStorage.getItem("espejo_config_oficina");
    if (cached) {
      try {
    ...
    return {
      diasActivos: [1, 2], // Default: Tuesdays (1) & Wednesdays (2)
      excluirJulioAgosto: true
    };
  });
  ...
  const getDaysOfWeek = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({
        ...
        mesIndex: d.getMonth(), // 0 = Jan, 6 = Jul, 7 = Aug
        diaSemanaIndex: i // 0 = Lunes, 1 = Martes, 2 = Miércoles...
      });
    }
    return days;
  };
  ...
  const isOfficeDay = (dia: { mesIndex?: number; diaSemanaIndex?: number }) => {
    if (dia.mesIndex === undefined || dia.diaSemanaIndex === undefined) return false;
    const isDayActive = configOficina.diasActivos.includes(dia.diaSemanaIndex);
    if (!isDayActive) return false;
    
    if (configOficina.excluirJulioAgosto) {
      if (dia.mesIndex === 6 || dia.mesIndex === 7) {
        return false;
      }
    }
    return true;
  ...
  const sugerirLookOficina = () => {
    ...
    const prendasOficina = armario.filter(p => getArmariosDePrenda(p).includes("oficina"));
    
    const tops = prendasOficina.filter(p => p.categoria === "top");
    const pantalones = prendasOficina.filter(p => p.categoria === "pantalon");
    const calzados = prendasOficina.filter(p => p.categoria === "calzado");
    const accesorios = prendasOficina.filter(p => p.categoria === "accesorio");

    // Fallback if office capsule is empty
    const finalTops = tops.length > 0 ? tops : armario.filter(p => p.categoria === "top");
    const finalPants = pantalones.length > 0 ? pantalones : armario.filter(p => p.categoria === "pantalon");
    const finalCalzados = calzados.length > 0 ? calzados : armario.filter(p => p.categoria === "calzado");
    const finalAccs = accesorios.length > 0 ? accesorios : armario.filter(p => p.categoria === "accesorio");

    const selectedPrendas: Prenda[] = [];
    
    if (finalTops.length > 0) {
      const randomTop = finalTops[Math.floor(Math.random() * finalTops.length)];
      selectedPrendas.push(randomTop);
    }
    if (finalPants.length > 0) {
      const randomPant = finalPants[Math.floor(Math.random() * finalPants.length)];
      selectedPrendas.push(randomPant);
    }
    if (finalCalzados.length > 0) {
      const randomCalzado = finalCalzados[Math.floor(Math.random() * finalCalzados.length)];
      selectedPrendas.push(randomCalzado);
    }
    if (finalAccs.length > 0 && Math.random() > 0.5) {
      const randomAcc = finalAccs[Math.floor(Math.random() * finalAccs.length)];
      selectedPrendas.push(randomAcc);
    }

    return {
      prendas: selectedPrendas,
      usandoCapsulaReal: tops.length > 0 || pantalones.length > 0 || calzados.length > 0
    };
  };

  const handleAutoSugerirOficina = (fechaStr: string, diaNombre: string, temp: number, clima: "soleado" | "nublado" | "lluvioso" | "tormenta" | "frio") => {
    const lookSugerido = sugerirLookOficina();
    if (lookSugerido.prendas.length === 0) {
      alert("No hay suficientes prendas cargadas en tu vestidor digital para sugerir un look.");
      return;
    }
    ...
    const newPlan: LookPlanificado = {
      id: "plan_" + Date.now(),
      fecha: fechaStr,
      nombre_look: `Look Oficina - ${diaNombre}`,
      prendasIds: lookSugerido.prendas.map(p => p.id),
      clima_simulado: {
        temp: temp,
        condicion: clima,
        ciudad: selectedCity
    ...
    if (!lookSugerido.usandoCapsulaReal) {
      newPlan.comentarios_sastre = "⚠️ Nota: Tu armario de 'Oficina' está vacío. Hemos sugerido prendas de tu vestidor general. ¡Añade prendas a la cápsula de oficina en tu armario! " + newPlan.comentarios_sastre;
    } else {
      newPlan.comentarios_sastre = "💼 Outfit sugerido automáticamente desde tu armario cápsula de Oficina. " + newPlan.comentarios_sastre;
    }
  ...
  return (
    <div className="space-y-6">
            ...
            <button
              ...
              className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition ${
                selectedCity === city.nombre
                  ? "bg-laton text-fondo"
                  : "bg-tarjeta text-tinta-apagada hover:text-tinta hover:bg-[#F4F4F5] border border-linea"
              }`}
            >
      ...
      {/* OFFICE DAYS RECURRENT CONFIGURATION PANEL - Marie Kondo elegant, minimal */}
      <div className="bg-tarjeta border border-linea rounded-xl p-4 space-y-3 shadow-sm">
        <div className="flex items-center gap-2 border-b border-linea pb-2">
          <Briefcase size={16} className="text-laton" />
          <h3 className="font-serif text-sm font-bold text-tinta">Configuración Recurrente de Días de Oficina</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
          <div className="space-y-2">
            <span className="font-bold text-[10px] text-laton uppercase tracking-wider block">Días de la semana obligatorios:</span>
            <div className="flex flex-wrap gap-1.5">
              {["Lunes", "Martes", "Miércoles", "Jueves", "Viernes", "Sábado", "Domingo"].map((name, i) => {
                const isActive = configOficina.diasActivos.includes(i);
                return (
                  <button
                    key={name}
                    type="button"
                    onClick={() => {
                      const updated = isActive
                        ? configOficina.diasActivos.filter((d: number) => d !== i)
                        : [...configOficina.diasActivos, i];
                      const newConfig = { ...configOficina, diasActivos: updated };
                      setConfigOficina(newConfig);
                      localStorage.setItem("espejo_config_oficina", JSON.stringify(newConfig));
                    }}
                    className={`px-2.5 py-1 rounded-full text-[10px] font-bold transition ${
                      isActive
                        ? "bg-laton text-fondo"
                        : "bg-[#F4F4F5] text-tinta-apagada hover:text-tinta border border-linea"
                    }`}
                  >
                    {name}
                  </button>
          ...
          <div className="space-y-3 flex flex-col justify-center">
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={configOficina.excluirJulioAgosto}
                onChange={(e) => {
                  const newConfig = { ...configOficina, excluirJulioAgosto: e.target.checked };
                  setConfigOficina(newConfig);
                  localStorage.setItem("espejo_config_oficina", JSON.stringify(newConfig));
                }}
                className="w-4 h-4 rounded border-linea text-laton focus:ring-laton"
              />
              <span className="font-medium text-tinta">Excluir meses de verano (Julio y Agosto)</span>
            </label>
            <p className="text-[10.5px] text-tinta-apagada leading-snug">
              Los días marcados se etiquetarán automáticamente en tu agenda como <strong>Días de Oficina</strong> para recomendarte looks de tu armario cápsula exclusivo.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-7 gap-3">
        {diasSemana.map((dia, index) => {
          ...
          const isOffice = isOfficeDay(dia);

          return (
            <div
              key={dia.fechaStr}
              className={`rounded-lg border p-3 flex flex-col justify-between min-h-[240px] transition duration-200 relative ${
                isToday
                  ? "bg-laton/5 border-laton shadow-md"
                  : "bg-tarjeta border-linea hover:border-laton-apagado"
              }`}
            >
              {/* Day header */}
              <div>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="flex items-baseline gap-1">
                      <span className="text-lg font-serif font-extrabold text-tinta">
                        {dia.diaMes}
                      </span>
                      <span className="text-[8px] font-mono text-tinta-apagada">
                        {dia.mesName}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="text-right">
                    <div className={`p-1 rounded ${CLIMAS_INFO[climaDia.condicion]?.bg || "bg-black/5"}`} title={CLIMAS_INFO[climaDia.condicion]?.label}>
                      <ClimaIcon size={12} className={CLIMAS_INFO[climaDia.condicion]?.color + " mx-auto"} />
                      <span className="text-[8.5px] font-mono text-tinta block mt-0.5">{climaDia.temp}°C</span>
                    </div>
                  </div>
                </div>
                ...
                {isOffice && (
                  <div className="mt-1.5 inline-flex items-center gap-1 bg-[#18181B]/10 text-tinta px-1.5 py-0.5 rounded text-[8.5px] font-bold uppercase tracking-wider">
                    <Briefcase size={8} />
                    <span>Oficina</span>
                  </div>
                )}
                  ...
                  {planesDia.length === 0 ? (
                    <div className="space-y-1.5">
                      <button
                        ...
                        className="w-full py-2 border border-dashed border-linea hover:border-laton/50 rounded flex flex-col items-center justify-center gap-1 text-[9px] font-bold uppercase tracking-wider text-tinta-apagada hover:text-tinta transition duration-150"
                      >
                      ...
                      {isOffice && (
                        <button
                          onClick={() => handleAutoSugerirOficina(dia.fechaStr, dia.nombre, climaDia.temp, climaDia.condicion)}
                          className="w-full py-1.5 bg-laton hover:bg-laton-apagado text-fondo rounded flex items-center justify-center gap-1 text-[9px] font-bold uppercase tracking-wider transition duration-150 shadow-sm"
                          title="Sugerir automáticamente un look sintonizado con tu armario cápsula de Oficina"
                        >
                          <Sparkles size={10} className="text-amber-300 animate-pulse" />
                          <span>Sugerir Oficina</span>
                        </button>
                      )}
                    </div>
                  ) : (
                    planesDia.map((plan) => {
                      ...
                      return (
                        <div key={plan.id} className="p-2 bg-[#F4F4F5] border border-linea rounded text-left space-y-2 relative">
                          <div>
                            <p className="text-[10px] font-bold text-tinta uppercase tracking-wider line-clamp-1">{plan.nombre_look}</p>
                          ...
                          <div className="flex flex-wrap gap-1">
                            {planPrendas.map(pr => (
                              <div
                                key={pr.id}
                                className="w-5 h-5 rounded border border-linea bg-tarjeta flex items-center justify-center overflow-hidden"
                                title={pr.nombre}
                              >
                                {pr.imageSrc ? (
                          ...
                          {plan.comentarios_sastre && (
                            <div className="text-[8.5px] text-tinta-apagada leading-snug bg-tarjeta p-1.5 border border-linea/60 rounded italic font-serif">
                              {plan.comentarios_sastre}
                            </div>
                          )}

                          {/* Interactions */}
                          <div className="flex items-center justify-between pt-1 border-t border-linea/60 gap-1.5">
                            <button
                              onClick={() => {
                                onMarcarComoVestido(plan.prendasIds);
                                alert("¡Prendas marcadas como puestas! Su contador de usos se incrementó en +1 y su Coste-por-Uso se recalculó.");
                              }}
                              className="text-[8px] bg-laton hover:bg-laton-apagado text-fondo font-bold uppercase tracking-wider px-1.5 py-0.5 rounded transition flex items-center gap-0.5"
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-tarjeta border border-linea rounded-xl p-5 max-w-2xl w-full max-h-[85vh] overflow-y-auto no-scrollbar space-y-4 shadow-xl"
            >
              <div className="flex justify-between items-center border-b border-linea pb-3">
                ...
                <button
                  ...
                  className="p-1 text-tinta-apagada hover:text-tinta border border-linea rounded text-xs"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-[#F4F4F5] border border-linea rounded text-xs text-tinta focus:outline-none focus:border-laton"
                />
              </div>

              {/* AUTOMATED OFFICE SUGGESTION INTEGRATION INSIDE MODAL */}
              {isOfficeDay(diasSemana.find(d => d.fechaStr === modalDate) || { mesIndex: -1, diaSemanaIndex: -1 }) && (
                <div className="bg-laton/5 border border-laton/15 p-3 rounded-lg flex items-center justify-between gap-3">
                  <div className="space-y-0.5">
                    <span className="text-[9px] uppercase tracking-wider text-laton font-bold block flex items-center gap-1">
                      <Briefcase size={11} /> Día de Oficina Detectado
                    </span>
                    <p className="text-[10px] text-tinta-apagada leading-normal">
                      Sintoniza este plan de forma automática con prendas de tu armario cápsula de Oficina.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      const lookSugerido = sugerirLookOficina();
                      if (lookSugerido.prendas.length > 0) {
                        setModalTitle(`Look Oficina - ${diasSemana.find(d => d.fechaStr === modalDate)?.nombre || ""}`);
                        setSelectedPrendasIds(lookSugerido.prendas.map(p => p.id));
                        if (!lookSugerido.usandoCapsulaReal) {
                          alert("¡Sugerencia cargada desde tu armario general! Clasifica prendas como 'oficina' en tu armario para mayor precisión.");
                        } else {
                          alert("¡Sugerencia cargada de forma óptima desde tu armario cápsula de Oficina!");
                        }
                      } else {
                        alert("No tienes prendas en tu vestidor para sugerir.");
                      }
                    }}
                    className="py-1 px-2.5 bg-laton hover:bg-laton-apagado text-fondo font-bold uppercase tracking-wider rounded text-[9px] transition shrink-0"
                  >
                    Cargar Sugerencia
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-[#F4F4F5] border border-linea p-3 rounded">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-tarjeta border border-linea rounded p-1.5 text-xs text-tinta focus:outline-none"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-tarjeta border border-linea rounded p-1.5 text-xs text-tinta font-mono focus:outline-none"
                  />
                </div>
              </div>
                  ...
                  <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                    {historial.map((item) => (
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-tarjeta border border-linea hover:border-laton rounded text-[10px] text-tinta shrink-0 transition text-left"
                      >
              ...
              <div className="space-y-2">
                ...
                {armario.length === 0 ? (
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 max-h-[220px] overflow-y-auto pr-1 no-scrollbar border border-linea p-2 rounded bg-[#F4F4F5]">
                    {armario.map((pr) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-2 rounded border cursor-pointer transition flex items-center gap-2.5 ${
                            isSelected ? "border-laton bg-laton/5" : "border-linea bg-tarjeta hover:bg-[#F4F4F5]"
                          }`}
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="text-[9.5px] text-tinta truncate leading-tight font-medium">{pr.nombre}</p>
                            <span className="text-[7.5px] font-mono text-tinta-apagada block uppercase leading-none">{pr.categoria}</span>
                          </div>
                          <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center shrink-0 ${
                            isSelected ? "border-laton bg-laton text-fondo" : "border-linea bg-black/5"
                          }`}>
              ...
              <div className="flex gap-2 justify-end pt-3 border-t border-linea">
                <button
                  type="button"
                  onClick={() => setShowModal(false)}
                  className="px-4 py-2 border border-linea text-tinta-apagada hover:text-tinta rounded text-xs font-bold uppercase tracking-wider"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-4 py-2 bg-laton hover:bg-laton-apagado text-fondo font-bold rounded text-xs uppercase tracking-wider flex items-center gap-1.5"
                >
